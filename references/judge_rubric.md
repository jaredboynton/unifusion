...
Two independent attempts expose each other's bugs: a bug one panelist made, the other often didn't, so a
merge that keeps what demonstrably ran ends up more correct than either input. The scrutiny lands hardest
at the seams (step 4), where a careless merge silently breaks.
...
whether any got there by a different route. One caveat when a shared session-context brief was injected
(see `panel.md`): agreement that merely restates a fact handed to all of them in that brief is correlated,
not independent, so weight it as a shared input rather than as convergent confirmation. Agreement the
panelists reached by their own research or reasoning still counts at full strength.
...
- Evidence over assertion: a panelist that ran the code or read the primary source outranks one reasoning
  from memory, regardless of model. Panelists are instructed to cite the file:line, command output, or
  source URL behind each claim; a cited, checkable claim outranks a bare assertion, and a claim a panelist
  itself flagged unverified should be treated as unverified unless another panelist substantiates it.
- Discount correlated agreement: when a shared session-context brief was injected, consensus that only
  echoes that brief is a shared input, not independent confirmation (see Consensus, above).
